...
const MovieSearchApp = () => {
  ...
  const debounceValue = useDebounce(values.search);
  useEffect(() => {
    async function fetchingData() {
      const res = await axios.get(
        `https://api.themoviedb.org/3/search/movie?query=${debounceValue}&api_key=36dcf302017b944213b4ddb61f639c29`
      );
    ...
    <div className="p-10">
      ...
      <div className="grid grid-cols-3 gap-10">
        {!isLoading &&
          movies.length > 0 &&
          movies.map((item) => {
            return <MovieItem data={item} key={item.id}></MovieItem>;
          })}
      </div>
    </div>
  );
    ...
    <div className="bg-white p-3 rounded-2xl shadow-sm flex flex-col">
      ...
      <div className="p-7 flex-1 flex flex-col">
        <h3 className="text-lg text-black font-semibold mb-4">
          <LoadingSkeleton height="28px"></LoadingSkeleton>
        </h3>
        ...
        <div className="flex items-center gap-x-3 mt-auto">
          ...
          <span className="text-sm font-semibold text-[#333]">
            <LoadingSkeleton width="100px" height="7px"></LoadingSkeleton>
          </span>
        </div>
      </div>
    </div>
